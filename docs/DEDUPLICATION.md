...
3. **В аналитических curated таблицах:**
   - PostgreSQL loader использует `source_system + procedure_number + lot_number` для lot lookup;
   - это не отменяет cross-source dedupe для чистой source sprint статистики.

4. **В SQLite демо-базе (`purchase_analysis.db`):**
   - импортируется уже задедуплицированный `output/merged_sprints.csv`;
   - дополнительный dedupe на уровне БД не применяется;
   - все VIEW работают с финальными 3 161 строками.

## Текущая чистая статистика

Allowlist: `configs/source_sprints_allowlist.csv`; manifest: `configs/source_sprints_manifest.csv`.

| Batch | Строк | Бюджет |
|---|---:|---:|
| `ast-full-2024-2025-finalcheck` | 2 761 | ~30 млрд ₽ |
| `b2b_center_prompt2_full_scope_2026-06-18` | 400 | ~0,2 млрд ₽ |
...
Итог: **3 161 уникальная закупка, 30,5 млрд ₽**.

## Почему всё равно нужен allowlist

После финальной уборки в `output/source_sprints` оставлены только clean batch-и. Allowlist остаётся обязательным контрактом: новый экспериментальный batch не должен попасть в статистику просто потому, что появился в директории.

Старый подход `merge all directories` удалён. `scripts/merge_sprints.py` по умолчанию читает только `configs/source_sprints_allowlist.csv`; `--all` оставлен только для диагностики.
