...
from purchase_analysis.utils.text import token_category
...
def build_procurements_frame(
    ...
    base_price = pd.to_numeric(df["price_rub"], errors="coerce")
    if "detail_price_rub" in df.columns:
        detail_price = pd.to_numeric(df["detail_price_rub"], errors="coerce")
        df["price_rub"] = base_price.combine_first(detail_price)
    else:
...
def build_procurement_items_frame(lots_df: pd.DataFrame) -> pd.DataFrame:
    if lots_df.empty:
        return pd.DataFrame()
...
def build_external_factors_frame(
    ...
    if usd_df.empty and key_df.empty:
        return pd.DataFrame()
...
def build_quality_summary(
    ...
    return {
        ...
        "coverage_note": (
            ...
            "sample; ZakazRF and Lot-Online are reproduced as exact probe sources."
        ),
    }
