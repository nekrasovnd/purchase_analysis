...
| **ЕИС** | ⚠️ 3 строки (дубли AST) | REST API | 3 процедуры найдены, но все уже присутствуют в AST-батче — удалены как cross-source дубли |
| **Tektorg** | ❌ 0 результатов | SOAP API | ИНН юрлиц группы Сбер не встречаются заказчиками ни в одной процедуре на площадке |
| **Roseltorg** | ❌ 0 результатов | REST API | Аналогично: API работает, но ни один ИНН из 32 не даёт результатов как заказчик/организатор |
| **TenderPro** | ❌ 0 результатов | REST API | Профили юрлиц найдены в реестре, но закупочных процедур нет |
| **LotOnline** | ❌ 0 результатов | JSON API | Промышленная b2b-площадка, юрлица Сбера не размещают на ней закупки |
| **ETP GPB** | ❌ 0 закупок | REST API | INN-фильтр возвращает имущественные торги (банкротства/залоговое имущество), не закупочные процедуры |
| **ZakazRF** | ❌ 0 процедур | HTML-парсинг | 3 юрлица найдены в реестре, но по всем трём — 0 размещённых процедур |
| **RTS Tender** | ❌ недоступен | HTTP | HTTP 503 на всех эндпоинтах во время аудита — площадка недоступна |
| **Fabrikant** | ❌ заблокирован | Next.js RSC / Server Action | Требует верификации юрлица на площадке. 4 попытки обхода: RSC GET (INN-параметр игнорируется сервером), Server Action `fetchOrganizationBySearch` (403 без авторизации), подделка cookie (403), текстовый поиск (10 из 102 результатов без пагинации API) |
